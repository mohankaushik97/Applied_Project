...
while i<=(length(emg_rms)-wind) 
    if (max(emg_rms(i-wind:i)))<=threshold && min(emg_rms(i+10:i+100)>= threshold)
        spike_emg(k)= emg_rms(i-1000);
        spike_idx(k) = Xs(i-1000);
        i = i+1000;
        k = k+1;
    else 
        i = i+1;
...
ones1 = ones(size(spike_idx));
ones2 = ones(size(acc_idx));

figure;
plot(spike_idx,ones1,'xr');
hold on;
plot(acc_idx,ones2,'+k');
hold off;
legend('emg','acc');